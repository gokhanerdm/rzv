-- Kişi kartına yapay zekânın yazdığı bilgilendirme (Gökhan, 2026-08-15: "kişi kartındaki
-- not kısmına kart sahibiyle ilgili bilgilendirme yapsın yapay zeka... yorum ve öneride
-- katsın... bir kez olsun her ziyarette tazelensin").
--   ai_ozet        : yazının kendisi
--   ai_ozet_kayit  : yazı üretildiği andaki toplam kayıt sayısı — bu sayı değişince
--                    (yeni rezervasyon/ziyaret) ekran yazıyı yeniden ürettirir
--   ai_ozet_tarih  : ne zaman üretildiği
alter table kisi_kartlari add column if not exists ai_ozet text;
alter table kisi_kartlari add column if not exists ai_ozet_kayit int;
alter table kisi_kartlari add column if not exists ai_ozet_tarih timestamptz;

drop function if exists kisi_karti_getir(uuid, text, uuid);

create function kisi_karti_getir(p_restaurant uuid, p_phone text, p_kisi_karti_id uuid default null)
returns table (
  kart_id uuid,
  isim text,
  kart_notu text,
