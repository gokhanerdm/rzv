-- Rezervasyon dışı (kapıdan) giriş de isim aramasından seçilebildiği için aynı kisi_karti_id
-- bağını burada da kurmak gerekiyor (Gökhan: "form o müşterinin ID'siyle devam etsin"). Yeni
-- parametre eklendiği için eski 5 parametreli sürüm PostgREST'te belirsizlik yaratmasın diye
-- önce düşürülüyor.
drop function if exists check_in_arrival(uuid, text, int, text, text);
...
begin
  ...
  if v_id is null then
    insert into reservations (restaurant_id, guest_name, guest_phone, party_size, reserved_at, status, arrived_at, note, consent_at, source, kisi_karti_id)
    values (
