-- Kişi kartını derinleştirme (Gökhan, 2026-08-07): "işletmenin bilmesi gereken her bilgi ve
-- istatistik olacak" — doğum günü, VIP, ilk KAYIT tarihi (ilk GELİŞten farklı: rezervasyon hiç
-- gelmemiş olsa bile ilk ne zaman yazıldığı), son rezervasyonun durumu, ve section 4 için TÜM
-- geçmiş (sadece gelişler değil, iptal/gelmedi dahil, en fazla 30 kayıt).
--
-- p_kisi_karti_id eklendi: isim aramasından seçilen kişi artık DOĞRUDAN ID'siyle aranabiliyor
-- (Gökhan: "form o müşterinin ID'siyle devam etsin, sadece isim/telefon metniyle eşleşmesin").
-- ID verilmişse öncelik onda; verilmemişse (eski çağrılar) telefon eşleşmesine düşer.
...
begin
  ...
  select
    ...
    (select coalesce(jsonb_agg(to_jsonb(x) order by x.reserved_at desc), '[]'::jsonb) from (
       select k.reserved_at, k.party_size, k.note, k.status, k.cancel_reason, t.name as masa
       from kayitlar k
