...
CREATE OR REPLACE FUNCTION public.labor_compliance(p_restaurant uuid, p_week_start date)
 RETURNS TABLE(staff_id uuid, full_name text, role text, hire_date date, week_hours numeric, overtime_hours numeric, has_consent boolean, year_overtime_hours numeric, leave_entitled integer, leave_used integer, leave_remaining integer)
 LANGUAGE sql
 STABLE
AS $function$
  with haftalik as (
